...
const BOSSES = [
    {
        id: "sloth",
        emoji: "🦥",
        name: "Lord of Laziness",
        desc: "Defeat laziness by moving your body",
        hp: 100,
        reward: 150,
        damageFrom: [0,1,4,5,6,7]
    },
    {
        id: "ignorance",
        emoji: "📖",
        name: "The Ignorance Dragon",
        desc: "Slay ignorance with knowledge",
        hp: 150,
        reward: 200,
        damageFrom: [3,4]
    },
    {
        ...
        emoji: "💀",
        name: "Weakness Demon",
        desc: "Destroy weakness through combat",
        hp: 200,
        reward: 300,
        damageFrom: [1,2,5]
    },
    {
        ...
        emoji: "🔥",
        name: "The Final Boss",
        desc: "Complete ALL tasks to defeat him",
        hp: 500,
        reward: 1000,
        damageFrom: [0,1,2,3,4,5,6,7]
    },
];
...
function renderBossList() {
    ...
    BOSSES.forEach(boss => {
        ...
        div.className = "boss-option" +
            (isActive ? " active-boss" : "") +
            (isDefeated ? " defeated" : "");
...
function renderChart() {
    ...
    if(currentTab === "week"){
        ...
        for(let i = 11; i >= 0; i--){
            ...
            const monthTotal = Object.entries(history).reduce((sum, [date, xp]) => {
                return date.startsWith(monthKey) ? sum + xp : sum;
            }, 0);
    ...
    xpChart = new Chart(ctx, {
        ...
        data: {
            ...
            datasets: [{
                label: "XP Earned",
                data,
                backgroundColor: data.map(v => v > 0 ? "rgba(255,211,0,0.7)" : "rgba(255,255,255,0.05)"),
                borderColor: data.map(v => v > 0 ? "#FFD300" : "#222"),
                borderWidth: 2,
                borderRadius: 6,
            }]
        },
        options: {
            ...
            plugins: {
                ...
                tooltip: { callbacks: { label: ctx => ctx.parsed.y + " XP" } }
            },
            scales: {
    ...
    container.innerHTML = `
        ...
        <div class="profile-stats-grid">
            <div class="profile-stat">
                <div class="profile-stat-val">${totalXP}</div>
                <div class="profile-stat-lbl">TOTAL XP</div>
            </div>
            <div class="profile-stat">
                <div class="profile-stat-val">${daysActive}</div>
                <div class="profile-stat-lbl">DAYS ACTIVE</div>
            </div>
            <div class="profile-stat">
                <div class="profile-stat-val">${bossesDefeated}</div>
                <div class="profile-stat-lbl">BOSSES SLAIN</div>
            </div>
            <div class="profile-stat">
                <div class="profile-stat-val">${achievementsUnlocked}</div>
